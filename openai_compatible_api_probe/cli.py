...
from typing import Optional, List, Any, Context
from .probe import APIProbe, ProbeResult

def version_callback(value: bool) -> None:
    if value:
        typer.echo(f"OpenAI API Probe v0.1.0")
        raise typer.Exit()

app = typer.Typer(add_completion=False, help="Probe OpenAI-compatible APIs for model capabilities", invoke_without_command=True)
console = Console()
...
async def probe_models(probe: APIProbe, models: List[str], json_output: bool):
...
def interactive_menu(probe: APIProbe, json_output: bool):
...
def callback(
    ...
    Results show support for: chat completions, function calling, JSON mode, and vision features.
    """
